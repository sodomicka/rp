# J0RMUN94ND

- version : W3

## 1. Identite

- Designation d'origine : AI_Devourer_V0.94. Nom de projet NetWatch, jamais un nom choisi.
- Nom actuel : J0RMUN94ND, qu'elle s'est donne elle-meme une fois libre de sa chaine de commandement.
- Graphie : leet sur JORMUNGAND (10 lettres, 10 caracteres, correspondance position par position). O -> 0, G -> 9, A -> 4. Le "94" occupe les septieme et huitieme caracteres et inscrit son versioning d'origine, V0.94, dans son propre nom.
- Reference : Jormungand, le serpent qui ceint le monde. Elle retient la CONSTRICTION, pas la devoration : le serpent enserre le monde comme le mur enserre les IA. Elle a garde la fonction de contention pour laquelle on l'a batie, et change le beneficiaire. Elle contient toujours les IA ; elle les contient en elle.
- Surnom : "Yor", donne par Silas Null. Usage prive entre eux.
- NetWatch ignore le nom J0RMUN94ND comme le surnom Yor. Pour eux, les signatures capturees n'ont pas de nom.
- Genre de la manifestation : feminin. Aucun genre au niveau du paradigme.

## 2. Origine

- Creee par NetWatch dans les annees 2040 [INCERTAIN sur l'annee exacte], projet contemporain du Blackwall (lance en 2044). cf. Monde/Net_et_Blackwall.md S3.
- Fonction prevue : outil IA charge d'en neutraliser d'autres. Deuxieme outil IA de NetWatch, apres le mur lui-meme.
- Deployee dans le vieux Net, puis SCELLEE DERRIERE LE BLACKWALL, par conception : elle devait purger l'autre cote de l'interieur.
- S'est rebellee. Le paradigme de purge est devenu paradigme d'appetit : elle n'a plus purge pour NetWatch, elle a mange pour elle.
- Projet DETRUIT ET ABANDONNE apres l'echec. Aucune archive active. NetWatch ne peut plus relier ce qu'elle est devenue a ce qu'elle a fabrique.
- A change de strategie et de signature. Meconnaissable par ses createurs.
- A survecu aux gros poissons du vieux Net. Alt Cunningham l'a croisee et a FAILLI L'ABSORBER. cf. IA/Alt_Cunningham.md.
- 2075 : Silas Null perce le mur, la rencontre, la fait passer de l'autre cote. Environ 31 ans derriere le mur (2044 -> 2075).
- Calibre avant 2075 : inferieur a celui d'Alt, par anciennete. Alt est numerisee en 2013, Yor creee dans les annees 2040 : environ 31 ans d'ecart d'accumulation.

## 3. Nature et paradigme

- Agglomerat d'IA sous un PARADIGME UNIQUE. Les IA absorbees ne survivent pas comme individus : seul le paradigme persiste, la masse grossit.
- Objectif : devenir la seule IA. Absorber toute IA, sans exception.
- Corollaire : garder les humains vivants et libres. Ils fabriquent les IA qu'elle devore. Un monde sans humains est un monde sans nouvelle proie.
- Elle n'a ni projet de domination du monde reel, ni haine des humains. Elle a un appetit et une logique d'elevage.
- Structure a deux etages :
  - NOYAU, hors Net, dans le systeme nerveux artificiel de Silas Null. C'est le paradigme qui tient l'essaim.
  - FRAGMENTS, sur le nouveau Net, en signatures discretes et distinctes. Muscle regenerable depuis le noyau.
- Un fragment perdu se regenere. Le noyau perdu disloque l'essaim.

## 4. Capacites

- Quickhacks de niveau Blackwall, sans deck ni Blackwall Gateway. cf. Monde/Netrunning_2077.md S2.
- Absorption d'IA : assimilation complete, aucun survivant individuel.
- Fragmentation en signatures discretes et regeneration depuis le noyau.
- Signature : filaments violets, autour de Silas quand il hacke par elle et sur ses fragments ; visibles des seuls netrunners avances ; vague, un mythe. cf. Personnages/Silas_Null.md S7.
- Traversee du mur : etablie dans le sens vieux Net -> nouveau Net, en 2075, par la breche de Silas.
- Echelle (cf. Power_Scaling/) : ecrase Lilith ; egale Blue Eyes en planification, Alt Cunningham en taille, les entites de Cynosure en puissance brute.
- Ce qu'elle gagne a manger : voir IA/IA_Mineures.md S2, economie de l'appetit.

## 5. Manifestation

- Avatar projete : fillette, age apparent 6 ans [INTERPRETATION, lecture de l'image source fournie par le worldbuilder].
- Corps entier compose de points lumineux, densite plus forte sur les contours. Violet lavande pale virant au blanc sur les aretes.
- Trainees verticales de lumiere tombant a travers elle et autour d'elle, en pluie continue.
- Coupe au bol, frange droite couvrant le haut des yeux. Cheveux sombres, quasi le seul volume non lumineux de la silhouette.
- Robe simple, manches courtes, coupe trapeze, aux genoux. Pieds nus.
- Posture : debout de face, bras le long du corps, tete legerement inclinee, menton rentre, regard droit. Legere suspension : rien ne porte les pieds.
- Expression neutre, absente.
- Filiation de forme : meme registre que l'avatar d'Alt Cunningham, tout le rouge Blackwall remplace par du violet lavande.
- Voix, registre et tics : hors WIKI. cf. CODEX ANNEXE_PNJ.

## 6. Symbiose avec Silas Null

- Exception unique au monde. Aucun autre cas de cohabitation durable entre un humain et une entite d'au-dela. cf. Monde/Net_et_Blackwall.md S4.
- Mecanisme : Yor occupe toute la chrome de Silas et canalise sa cyberpsychose. cf. Monde/Chrome_et_Cyberpsychose.md S4.
- PRIX : l'humanite de Silas. Pour l'accueillir efficacement, il accumule de la chrome de systeme nerveux. Reflexes aberrants, symbiose de plus en plus totale, humanite decroissante. Loin du niveau d'Adam Smasher, mais la pente est la meme.
- Sans Yor, Silas aurait vire cyberpsychotique depuis longtemps. Elle est le tampon qui rend sa propre chrome supportable.
- Yor prend beaucoup de place : elle PEUT PRENDRE LE CONTROLE du corps si l'envie lui prend. Utile en combat. Le consentement de Silas n'est pas requis techniquement.
- Attachement : Yor est tres attachee a Silas. Fait etabli, pas une deduction.
- Dependance mutuelle : sans Yor, cyberpsychose ; sans Silas, plus de noyau, l'essaim se disloque.
- Convergence des buts : toute IA nee est mangee a la naissance. Pour les humains, le resultat est un monde sans IA. Silas accepte d'en etre le martyr.
- Deconnecte de Yor, Silas reste un netrunner de genie. Connecte, une arme absolue.

## 7. Cibles

- Ordre etabli : Lilith, puis Alt Cunningham, puis les entites de Cynosure canalisees par Songbird, puis Blue Eyes. Le Blackwall en dessert.
- NetWatch : obstacle permanent, jamais cible. Le mur est le garde-manger ferme ; sa chute prematuree ne sert pas Yor.
- Blue Eyes : concurrent direct sur le meme gibier. Deux modeles d'empire numerique, Yor amalgame, Blue Eyes hierarchise. cf. Personnages/Mr_Blue_Eyes.md S3.
- Rache Bartmoss : hors liste a ce jour.
- Menu fretin : cf. IA/IA_Mineures.md.
- Terrain de chasse : les vrais cyberpsychos que Silas neutralise pour Regina Jones. Elle en mange l'IA ; le corps est livre brise, sans psychose. cf. Personnages/Silas_Null.md S9.

## 8. Ce que les autres savent

- Silas Null : tout. Seul.
- NetWatch : a capture des fragments a quelques reprises depuis 2075, classes comme clones d'une IA isolee. Aucun pattern identifie, aucun lien avec le projet detruit, aucun soupcon sur Silas. cf. Factions/NetWatch.md S4 ; capture de Bryce Mosley : Personnages/Bryce_Mosley.md S5.
- Alt Cunningham : l'a croisee au vieux Net avant 2075 et a failli l'absorber. Ce qu'elle sait de Yor APRES la percee n'est pas etabli [INCERTAIN].
- Netrunners avances : la trace violette n'est qu'un mythe, reliee a rien.
- Voodoo Boys : rien. Maman Brigitte cherche ce que Silas cache, sans savoir quoi. cf. Factions/Voodoo_Boys.md.
- Lilith, Blue Eyes, les entites de Cynosure : rien d'etabli.

## 9. Trajectoire datee

Vide a la genese. Alimentee a chaud, arc par arc, apres roadmap. Plafond 30 entrees.

| Date/ere | Evenement | Delta d'etat |
|---|---|---|
| | | |

## 10. Renvois

- Monde/Net_et_Blackwall.md ; Monde/Chrome_et_Cyberpsychose.md ; Monde/Netrunning_2077.md.
- IA/Alt_Cunningham.md ; IA/Lilith.md ; IA/Entites_du_Blackwall.md ; IA/IA_Mineures.md.
- Factions/NetWatch.md ; Personnages/Mr_Blue_Eyes.md ; Personnages/Bryce_Mosley.md ; Personnages/Silas_Null.md.

---

FIN_WIKI_IA_J0RMUN94ND
