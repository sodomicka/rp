# Chronologie des roadmaps - Sonia / Jumelles d'Ohara

- version : W7
- note de version : W7 - R4 Alabasta ECRITE (scenes clefs dictees ; tous ses [A POSER] tranches : face-a-face Zoro = beat des couteaux, livraison Wapol en personne au casino fenetre Little Garden, frontiere Chopper = Alabasta et rien avant). Bornes R4/R5 ajustees : intronisation deplacee en R5 ; borne de sortie R4 = reveil de Sonia dans la cale du Going Merry. R3 bumpee W2 (retrofit : etape 7bis - visite de Robin a Miss Wednesday an -1, le pion arme, premiere separation volontaire ; etape 8 detaillee - Mr. 1 propose, Mr. 0 exige ; etape 9 amendee - revelation de l'infiltration a la demande, presentee en decouverte fraiche). Retombees versees : R5 (intronisation ; dog whistle Aokiji - radar reveille par le relink public d'Alubarna, trace remontee jusqu'a Long Ring Long Land ; seed Sanji renforce par plant explicite au casino), R6 ([A POSER] primes enrichi : relink public acte en R4), R9 (socle Ace : fiche NOYAU actee). Fiches en file de la boucle R4 : Vivi, Smoker, Tashigi, Ace (NOYAU) ; silhouettes actees : Wapol, Cobra, Pell, Koza, Daz Bonez, Officer Agents, Igaram (reconduit). W6 - R3 Baroque Works ECRITE (tous ses [A POSER] tranches au build : audience de Rainbase, nouvelle de Tom par la presse, organigramme et routine Miss Black Friday, hook Wapol adopte et route en R4). Retombees versees : R4 (Erumalu = confrontation, [A POSER] "consequence de la detonation" sans objet ; livraison Wapol), R9 (Armement appris a Baltigo aupres de Sabo et Koala), R12 (Tchekhov "Miroir Viola" : detonation, confidence de Sonia) ; repere de datation fondation BW balise [DIVERGENCE RP]. Roadmap_2 bumpee W3 (Puffing Tom en service an -12 ; Erumalu requalifiee). W5 - Roadmap_2 bumpee W2 (bloc Quotidien de l'annee Tom : duos Robin-Iceburg / Sonia-Cutty Flam, Iceburg feru d'histoire). W4 - R2 Grand Line ECRITE, tous ses [A POSER] tranches (couture Twin Capes sans escale ; titre ne a l'embuscade an -12 ; doctrine "aucun temoin" ; chute de Tom placee en R3) ; R1 bumpee W2 (retrofit : modus operandi, logement, regle de visibilite) ; Tchekhov Erumalu plante (R2 -> R4) ; retombees versees en R3, R4, R9. W3 - R1 West Blue ecrite ; import de perimetre Esperia. W2 - R0 Ohara ecrite (thread 12). W1 - genese, decoupage R0 -> R15 dicte. Grandes lignes seulement : chaque roadmap se detaille a son BUILD (boucle serree de Passe 2) ; les [A POSER] listes ici s'y tranchent.
- convention de nommage (actee) : chaque roadmap porte le nom du LIEU ou de l'etape de son arc. Exception acceptee : Baroque Works (R3).
- indexation (tranchee thread 12) : le garde cap SEUL est indexe au Sommaire (au prochain build de Sommaire) ; les roadmaps individuelles restent hors Sommaire, listees ici.
- prota / POV : Nico Sonia (les jumelles, jamais separees).

## Reperes de datation
- an 0 = depart de Luffy en mer (East Blue -> Marineford condense sur quelques mois in-world). Ellipse an 0 -> an +2.
- Point de depart du RP (t0) : Ohara, J-2 du Buster Call, an -20. Saul echoue a J-4 (t0 = le surlendemain).
- Jumelles nees an -28 : 8 ans a t0 ; 14 ans au passage en Grand Line (an -14) ; 16 ans a la naissance de la legende (an -12) ; 19 ans a l'arrivee a Water 7 (an -9) ; 20 ans a l'entree BW (an -8) ; 27 ans a la visite a Miss Wednesday (an -1) ; 28 ans a Enies Lobby (an 0) ; 30 ans a Egghead (an +2).
- Legende "Les Lumieres de la Revolution" : nee an -12 (embuscade, temoins revolutionnaires - Roadmap_2).
- Fondation de Baroque Works : vers an -10 au plus tot [DIVERGENCE RP - canon : fondation ~16 ans avant le present] (derive du plafond BIBLE "sejour BW sous 10 ans") ; Whiskey Peak deja tenue an -10/-9 (BW naissante).
- Execution de Roger : an -22.
- Attaque de Barbe Noire sur Drum : an 0, environ 2-3 mois avant l'arrivee des Mugiwara a Drum [canon verifie] - donc anterieure a Whiskey Peak. Fenetre de la livraison Wapol : cf. R4.

## Fils flottants (sans arc attitre)
- Carnet rouge : detonation et consequence a definir (P2).
- Vecteur Binks de Luffy (annee Shanks a Fuchsia, ou repris d'Usopp a bord) : question ouverte W6.

## R0 - Ohara [ECRITE - JOUABLE, intro du RP]
- Bornes : t0 (J-2 du Buster Call, an -20) -> sortie des jumelles hors des eaux d'Ohara.
- Detail complet : cf. Roadmap_0_Ohara.md - itineraire en 9 etapes, condition d'Aokiji (verbatim acte), casting, notes de frontiere. Tous les [A POSER] de genese tranches (thread 12).
- Fichier : Roadmap_0_Ohara.md

## R1 - West Blue [ECRITE - W2]
- Bornes : sortie des eaux d'Ohara (an -20, 8 ans) -> passage en Grand Line (an -14, 14 ans), descente de Reverse Mountain en cale.
- Detail complet : cf. Roadmap_1_West_Blue.md (W2) - itineraire en 7 etapes, boucle de cavale (pattern de jeu libre), modus operandi du croc-en-jambe au graphite, regime de logement, regle de survie en cale, motivation double du passage (necessite + vocation), casting OC (Bonace, Corvara, Murene - identites fixees, pas de fiches wiki), import de perimetre Esperia (lore passe uniquement).
- Jalons cles : avis "Les Enfants du Demon" (120M) des l'ouverture (mecanisme canon transpose : reperees au large de l'ile NE d'Ohara) ; variante Binks a double vecteur (coque de la Bonace / chant-piege de la Murene), lignee d'Esperia cousine des Rumbar - Tchekhov R7 ; premieres morts an -16/-15 (une chacune, deux Marines de la 80e Branche, aucun temoin, prime inchangee) ; Observation acquis en fil d'arc.
- Fichier : Roadmap_1_West_Blue.md

## R2 - Grand Line [ECRITE - W3]
- Bornes : descente de Reverse Mountain (an -14, 14 ans) -> fuite en douce de Water 7 (soir de l'annonce du CP5, an -8, 20 ans), cap sur Alabasta.
- Detail complet : cf. Roadmap_2_Grand_Line.md (W3) - itineraire en 9 etapes, regle de mobilite (allers-retours Paradis, appui Merry ~42 jours), doctrine "aucun temoin", nuit du hangar, annee Tom et son quotidien (duos actes : Robin-Iceburg les poses, Sonia-Cutty Flam les impulsifs), enquete BW bouclee.
- Jalons cles : aucune escale aux Twin Capes ; escale Alabasta an -13/-12 (Nanohana ; Erumalu vue VIVANTE - souvenir plante, requalifie R3 : R4 = confrontation, pas revelation) ; embuscade an -12 = naissance de la legende "Les Lumieres de la Revolution" (temoins revolutionnaires ; nom clandestin, jamais sur avis ; prime inchangee) ; Mock Town evitee ; Whiskey Peak an -10/-9 (BW naissante, enquete ouverte) ; Little Garden au large (geants en duel, echo Saul) ; San Faldo puis Water 7 an -9 (nuit du hangar : Iceburg surpris, Tom accueille en amies) ; annee Tom (Puffing Tom en service depuis an -12, chantier des dernieres lignes), enquete bouclee (bases BW = anciens repaires secrets de la Marine ; base principale = casino d'Alabasta) ; fuite an -8 avec DESTINATION (BW), pour proteger la maisonnee.
- Fichier : Roadmap_2_Grand_Line.md

## R3 - Baroque Works [ECRITE - W2]
- Bornes : recrutement BW (an -8, 20 ans) -> veille de Whiskey Peak (an 0, 28 ans), jumelles en route ou sur place sur accord de Mr. 0.
- Detail complet : cf. Roadmap_3_Baroque_Works.md (W2) - itineraire en 10 etapes (9 + 7bis), arrangement en lore plein (negociation menee par Sonia, scenes A et B, serment), triangle d'Observation, routine du bureau (moteur structurel de l'arc), audience de Rainbase dictee en detail. W2 (retrofit R4) : etape 7bis - visite de Robin a Miss Wednesday an -1 (le pion arme, fin du savoir a sens unique, premiere separation volontaire, une journee) ; operation Zoro detaillee ; etape 9 amendee (revelation a la demande). Doctrine actee au build : aucun incertain residuel, aucune roue libre - le MJ narre et invente DANS le cadre, jamais le cadre.
- Jalons cles : audience de Rainbase an -8 (pari corsaire/vice-amiral, arrangement pose par Sonia elle-meme, noms de code apportes prets) ; nouvelle de la chute de Tom an -8/-7 (presse, routine-journal ; deuil a huis clos) ; scene A an -5 ; pivot an -3 (lancement Dance Powder, dispute Robin-Crocodile, scene B, serment mort-ou-Impel-Down [plant Tchekhov "Crocodile libre"], patience arrachee) ; couverture Vivi/Igaram percee an -2 (savoir a sens unique, piece gardee) ; visite an -1 (7bis : le pion arme) ; operation Zoro an -1 (ex-Mr. 7 seul a Sixis ; Mr. 1 propose, Mr. 0 exige) ; an 0 : convergence Igaram/Vivi/Roronoa sur Whiskey Peak, demande accordee par Mr. 0, carte Vivi jouee.
- Fichier : Roadmap_3_Baroque_Works.md

## R4 - Alabasta [ECRITE - W1]
- Bornes : veille de Whiskey Peak (an 0, 28 ans) -> reveil de Sonia dans la cale du Going Merry, en mer apres Alubarna. Intronisation deplacee en R5.
- Detail complet : cf. Roadmap_4_Alabasta.md (W1) - itineraire en 8 etapes, scenes clefs dictees (Whiskey Peak a deux, casino Wapol et chambre, Pell epargne, route d'Alubarna, relink Tashigi, tombe des rois, cale), sections "La carte Vivi" et "L'armement prepare" (gourde, couteaux, antidote).
- Jalons cles : revelation de l'infiltration de Vivi a la demande (mecanisme Mr. 5 / Miss Valentine conserve ; double tableau : evaluer l'arme du serment ou patienter) ; Whiskey Peak - Robin mysterieuse / Sonia excentrique, chapeau (Hana Hana), beat des couteaux face a Zoro (dette Mr. 7 / Mr. 1), Eternal Pose en poirier ecrase par Luffy, depart en Banchi sous petales ; casino - livraison Wapol (fenetre Little Garden), couteau entre les cuisses, le soir : seed Sanji ("le blondinet") + aveu d'epuisement du corps-marchandise ; Rainbase - Pell abattu-epargne par Sonia (lance de graphite friable, aucun point vital) ; combat 1 - "les soeurs Nico" devant temoin, insolence puis diversion de Sonia, sauvetage de Luffy par Robin (question du D. conservee), Luffy confie a Pell ; Alubarna - RELINK PUBLIC (Tashigi les nomme), rage meurtriere de Sonia retenue sans un mot par Robin ("je peux la tuer ?" / "on n'a pas le temps") ; tombe des rois - bluff a deux sur la stele (fausse traduction de Sonia soutenue par Robin, pretes a mourir ; Cobra seul temoin lucide : Poseidon protege), Sonia contre le mur, "inutiles", antidote passe a Cobra pour Luffy, Luffy sort Sonia et Cobra de la tombe ; PREMIERE DETONATION du serment (exauce par un autre) ; fin de l'arrangement ; dog whistle Aokiji (radar reveille) ; Erumalu - confrontation muette (jalon requalifie tenu) ; cale du Merry (borne de sortie).
- Fichier : Roadmap_4_Alabasta.md

## R5 - Skypiea [A BUILDER]
- Bornes : cale du Going Merry, en mer apres Alubarna (an 0) -> rencontre Aokiji (post-Davy Back Fight, avant Water 7).
- Jalons : INTRONISATION des jumelles (deplacee de R4 : reveil en cale = borne d'entree ; premiere rencontre a decouvert avec l'equipage, invitation, banquet - a cadrer au build) ; beat Sanji-Sonia EN MER, apres Alabasta et avant Skypiea (dicte thread 12 ; garde-fous W6 : jamais nomme, jamais verbalise, jamais repete ; plant explicite au casino, R4) ; Jaya ; Skypiea - danse de Nika reconnue chez Luffy (feu de camp shandia, dansee avant qu'on la lui montre) ; Davy Back Fight ; fin d'arc : Aokiji - SUR LA TRACE depuis le relink d'Alubarna (dog whistle, acte R4) - congele LES DEUX jumelles pour tester la reaction de l'equipage et leur force ; il decide de les leur laisser et de n'en emporter aucune (dicte thread 12).
- Fichier : Roadmap_5_Skypiea.md

## R6 - Water 7 [A BUILDER]
- Bornes : arrivee a Water 7 (an 0) -> funerailles vikings du Going Merry.
- Jalons (ordre chrono) : retour dans la ville-refuge ; retrouvailles Franky / Iceburg / Kokoro - la maisonnee SAIT ("vous avez fui pour proteger Tom, et il est tombe quand meme") ; carnet rouge planque sur le Merry ; reddition volontaire des deux au CP9 (leurs vies plus Pluton pesent moins que Nika) ; Enies Lobby : drapeau du GM brule par l'equipage, cri a deux voix ("on veut vivre" / "venez nous sauver, les amis") ; mutilation du bras gauche de Sonia par Funkfreed ("une enfant du demon lui suffit") ; eveil du Kaki Kaki no Mi, Sonia mourante sauve Robin ; sauvetage par l'equipage - condition d'Aokiji REMPLIE ; plans de Pluton brules par Franky ; recuperation du carnet rouge (verif actee : AVANT les funerailles) ; funerailles du Merry ; Franky rejoint.
- [A POSER] : poids du retour dans la ville-refuge ; geste exact de la mutilation (arme actee : Funkfreed) ; attribution du Golden Escargophone a Spandam par Aokiji (placement) ; primes post-Enies Lobby (montant, commune ou scindee ; relink public des identites acte en R4, Alubarna) ; posture d'Aokiji apres coup (condition remplie).
- Fichier : Roadmap_6_Water_7.md

## R7 - Thriller Bark [A BUILDER]
- Bornes : entree dans le Triangle de Florian (an 0) -> depart de Thriller Bark.
- Jalons : Tactics 15 "Big Emperor" - Docking ACCOMPLI grace a Sonia (inversion du refus canon de Robin) ; "il ne s'est rien passe" de Kuma ; pacte Kuma-jumelles entre adeptes de Nika - dispersion future pre-acceptee, condition unique : jamais separees l'une de l'autre ; Brook rejoint (porteur de la culture Binks).
- [A POSER] : repartition exacte des places du Docking 6 ; piste Binks - la version des jumelles (lignee d'Esperia, actee R1) cousine de celle des Rumbar (resonance Brook / Laboon), detonation a cadrer.
- Fichier : Roadmap_7_Thriller_Bark.md

## R8 - Sabaody [A BUILDER]
- Bornes : arrivee sur l'archipel (an 0) -> dispersion de l'equipage par Kuma.
- Jalons : Rayleigh face a Kizaru = PREMIERE utilisation de Haki vue par l'equipage (dicte : aucun beat calme de revelation avant la separation) ; dispersion ; jumelles projetees ENSEMBLE vers Tequila Wolf (dicte thread 12 ; pacte Kuma honore).
- Fichier : Roadmap_8_Sabaody.md

## R9 - Baltigo [A BUILDER]
- Bornes : atterrissage a Tequila Wolf (an 0) -> depart pour le retour a Sabaody (an +2).
- Jalons : Tequila Wolf ; prise en charge par l'Armee Revolutionnaire, Baltigo ; elles arrivent DEJA TITREES (legende "Les Lumieres de la Revolution" nee an -12, temoins revolutionnaires - acte R2) ; nouvelles d'Impel Down et de Marineford - deuil d'Ace ; Crocodile libere par Luffy : crise privee des jumelles (nausees de Sonia, secret maintenu) - seconde detonation du serment (retourne) ; deux ans de formation - Armement appris aupres de Sabo et Koala (acte R3) [Haki de Koala : jamais atteste au canon - a baliser au build] ; Baltigo detruit APRES leur depart (an +2).
- [A POSER] : contenu de l'ellipse (formation, rapports avec Dragon) ; reception de la legende cote Armee (dossier, attente depuis an -12 ?).
- Fichier : Roadmap_9_Baltigo.md

## R10 - Hommes-Poissons [A BUILDER]
- Bornes : retrouvailles a Sabaody (an +2) -> depart de l'Ile des Hommes-Poissons.
- Jalons : premiere rencontre des jumelles avec Jinbe (frontiere actee).
- [A POSER] : revelation du Haki des jumelles aux Mugiwara (reporte de R8 : rien avant la separation ; premiere fenetre calme ici).
- Fichier : Roadmap_10_Hommes_Poissons.md

## R11 - Punk Hazard [A BUILDER]
- Bornes : entree dans le Nouveau Monde -> depart de Punk Hazard.
- Jalons : trame canon suivie par defaut ; AU a poser au build ; retrouvailles Smoker/Tashigi (fiches NOYAU creees a la boucle R4).
- Fichier : Roadmap_11_Punk_Hazard.md

## R12 - Dressrosa [A BUILDER]
- Bornes : arrivee a Dressrosa -> chute de Doflamingo, depart.
- Jalons : trame canon suivie par defaut ; AU a poser au build ; detonation du Tchekhov "Miroir Viola" (plant R3) - rencontre de Viola, confidence de Sonia sur l'arrangement (cercle du secret etendu, acte R3).
- [A POSER] : modalites de la rencontre et de la confidence ; consequence de la detonation.
- Fichier : Roadmap_12_Dressrosa.md

## R13 - Zou [A BUILDER]
- Bornes : arrivee sur Zunesha -> depart du groupe Wano.
- Jalons : trame canon suivie par defaut ; les jumelles partent avec le groupe Wano - pas de Whole Cake Island pour elles (dicte, conforme a la trajectoire canon de Robin).
- Fichier : Roadmap_13_Zou.md

## R14 - Wano [A BUILDER]
- Bornes : arrivee a Wano -> liberation de Wano.
- Jalons : infiltration des DEUX jumelles en geisha (dicte) ; Gear 5 revele - detonation du fil Foi Nika.
- [A POSER] : reaction des jumelles a la revelation ; gestion du secret Nika vis-a-vis de l'equipage.
- Fichier : Roadmap_14_Wano.md

## R15 - Egghead [A BUILDER - horizon de couverture]
- Bornes : arrivee a Egghead (an +2) -> fin d'arc (au-dela : canon en cours, hors perimetre).
- Jalons : Saul revele VIVANT et livres d'Ohara conserves sur Elbaf - revelation faite A EGGHEAD par Vegapunk (canon ch. 1066, conforme BIBLE SB3) ; affrontement Marine et CP0 ; sort de Kuma.
- Fichier : Roadmap_15_Egghead.md

FIN_ROADMAP_SONIA_CHRONOLOGIE
