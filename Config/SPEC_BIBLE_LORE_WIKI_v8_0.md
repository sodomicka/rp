...
Emplacement : `https://github.com/sodomicka/rp/blob/main/Config/SPEC_BIBLE_LORE_WIKI_v8_0.md`.
...
Le CODEX ANNEXE_CHRONO > Arcs pointe vers la page Roadmap concernee via `roadmap: cf. WIKI Roadmap/<Prota>/<page>.md`. La roadmap est par perspective de prota ; les decisions specifiques a un RP sont tracees dans `Parties/{Univers}/Partie<n>/Decisions/`.
...
SB9 ne contient PAS l'index detaille page-par-page. Cet index vit dans `{Univers}/Sommaire.md` (page WIKI fetchee au debut de chaque thread, cf. section 10), pour que SB9 reste de taille fixe et ne croisse plus avec le nombre de pages.
...
Bloc couvrant un long laps (annees/decennies) a rendre en vignettes : marquer la section `[SNAPSHOTS]`. Le MJ compresse alors en vignettes evocatrices au lieu de jouer chaque etape - compresser n'est PAS supprimer, le joueur doit sentir le temps passer (cf. Instructions RP, sequence de roadmap).
...
### Fetch
- Roadmap de l'arc en cours : fetch obligatoire en debut de thread RP depuis `{Univers}/Roadmap/<Prota>/`.
...
### Fetch de la SPEC
Au premier BIBLE BUILD (aucune BIBLE existante), le MJ fetche `https://github.com/sodomicka/rp/blob/main/Config/SPEC_BIBLE_LORE_WIKI_v8_0.md`. Si la SPEC n'est pas encore sur le depot (tout premier build, rien n'existe), le joueur la fournit en fichier joint ou dans le chat pour ce premier thread. Apres ce premier build, la SPEC est uploadee dans Config/ et tout passe en fetch.
...
### Chargement par tour
1. BIBLE_LORE en entier (SB0-SB9) - obligatoire, c'est l'index.
2. Si la scene necessite un detail non couvert par SB0-SB8 : consulter SB9, identifier la page, construire l'URL, fetch.
...
3. Fetch de la page Roadmap de l'arc (`wiki_base` + `Roadmap/<Prota>/<page>.md`) - reste en contexte tout le thread.
4. Si le CODEX S3 renvoie a une fiche dans Parties/ : fetch optionnel de la fiche protagoniste.
5. Narration.
