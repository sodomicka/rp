...
3. BIBLE_LORE §B9 (index vers WIKI ou Parties ?) -> si oui, fetch de la page.
...
- Redaction SANS ACCENT (comme le CODEX).
...
| §B9 | 6-15k | Index WIKI + Parties (croissance avec le nombre de pages) |
| Journal | 1-3k | — |
...
#### §B9 Index du WIKI et des Parties [OBLIGATOIRE si WIKI present]
Table de routage vers les pages du WIKI et de Parties/.
...
Regles de §B9 :
- Chaque entree pointe vers un dossier et une page. L'URL se construit : `wiki_base` ou `parties_base` (§B0) + `<Dossier>/<Page>.md`.
- Chaque page est une unite autonome, fetchee en entier. Pas de lecture partielle.
- §B9 doit etre suffisamment descriptif pour que le MJ sache QUELLE page fetcher sans deviner.
- Les pages Parties/ sont signalees par un prefixe `[PARTIES]` ou listees dans une section §B9 separee.
...
2. Construire l'URL de la page Roadmap correspondante (`wiki_base` + `Roadmap/<page>.md`).
3. Fetch de cette page — elle reste en contexte pour tout le thread.
...
### <Dossier>/ (WIKI)
Description : <contenu en 1 ligne>
Pages :
